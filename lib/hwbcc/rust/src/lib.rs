...
/// Result of retrieving protected data.
#[derive(Debug)]
...
/// # Returns
///
/// A DataResult containing truncated prefixes of the populated
/// `cose_sign1` and `bcc` buffers. A [`HwBccError`] will be
/// returned if the user supplies an empty `cose_sign1` or AAD
/// that is larger than the supported size.
///
/// # Examples
///
/// ```
...
pub fn get_protected_data<'a>(
    ...
    cose_sign1: &'a mut [u8],
    bcc: &'a mut [u8],
) -> Result<DataResult<'a>, HwBccError> {
